...
class Torrent extends EventEmitter {
  constructor (torrentId, client, opts) {
    ...
    this._hashedPieces = 0
    this._storedPieces = 0
  ...
  get _unstartedPieces () {
    return this.pieces.filter(p => p && !p._reservations)
  }

  get _downloadedPieces () {
  ...
  _markVerified (index) {
    this._hashedPieces += 1
    this._storedPieces += 1
  ...
  _updateWire (wire) {
    ...
    this._debug(
      '_updateWire',
      wire._debugId, wire.peerChoking, wire.amInterested, wire.downloaded !== 0,
      self._unstartedPieces.length, self._downloadedPieces.length,
      self._hashedPieces, self._storedPieces,
    )

    if (wire.peerChoking) return
    if (!wire.amInterested) return
    if (!wire.downloaded) return validateWire()

    const minOutstandingRequests = getBlockPipelineLength(wire, PIPELINE_MIN_DURATION)
    if (wire.requests.length >= minOutstandingRequests) return
    let maxOutstandingRequests = getBlockPipelineLength(wire, PIPELINE_MAX_DURATION)

    if (!self._unstartedPieces.length) return
    if (self._downloadedPieces.length > 2 + self._hashedPieces) return
    if (self._hashedPieces > 2 + self._storedPieces) return

    trySelectWire(false) || trySelectWire(true)

    function genPieceFilterFunc (start, end, tried, rank) {
      return i => i >= start && i <= end && !(i in tried) && !self.bitfield.get(i) && wire.peerPieces.get(i) && (!rank || rank(i))
    }
    ...
    function trySelectWire (hotswap) {
      ...
      for (let i = 0; i < self._selections.length; i++) {
        ...
        let piece
        if (self.strategy === 'rarest') {
          // request pieces in progress with unreserved block
          for (const [index, piece] of self.pieces.entries()) {
            if (!(piece && piece._reservations && piece._reservations !== piece._chunck)) continue
            if (wire.peerPieces.get(index)) {
              if (self._request(wire, index, self._critical[index] || hotswap)) {
                while (self._request(wire, index, self._critical[index] || hotswap)) {}
  ...
  _request (wire, index, hotswap) {
    ...
    wire.request(index, chunkOffset, chunkLength, function onChunk (err, chunk) {
      ...
      if (piece !== self.pieces[index]) return self._updateWireWrapper(wire)
      ...
      if (err) {
        ...
        return self._updateWireWrapper(wire)
      }
      ...
      if (!piece.set(reservation, chunk, wire)) return self._updateWireWrapper(wire)
