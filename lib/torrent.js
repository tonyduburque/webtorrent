...
class Torrent extends EventEmitter {
  ...
  _updateInterest () {
    ...
    this.wires.forEach(wire => {
      interested = this._updateWireInterest(wire);
    });
  ...
  _updateWire (wire) {
    ...
    while(wire.requests.length < maxOutstandingRequests) {
      if (!wire.amInterested) return;
      if (!(trySelectWire(false) || trySelectWire(true))) return;
      maxOutstandingRequests = getBlockPipelineLength(wire, PIPELINE_MAX_DURATION);
    }
